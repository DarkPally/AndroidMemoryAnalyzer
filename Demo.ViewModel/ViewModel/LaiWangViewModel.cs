...
using Demo.Library.LaiWang;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class LaiWangViewModel : BindableBase
    {

        List<LaiWangMessage> msgs;
        public List<LaiWangMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = LaiWangHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
