...
using Demo.Library.Fetion;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class FetionViewModel : BindableBase
    {

        List<FetionMessage> msgs;
        public List<FetionMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = FetionHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
