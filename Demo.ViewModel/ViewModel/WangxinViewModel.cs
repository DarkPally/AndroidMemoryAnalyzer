...
using Demo.Library.Wangxin;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class WangxinViewModel : BindableBase
    {

        List<WangxinMessage> msgs;
        public List<WangxinMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = WangxinHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
