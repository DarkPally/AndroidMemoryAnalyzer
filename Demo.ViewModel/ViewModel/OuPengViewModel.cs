...
using Demo.Library.OuPeng;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class OuPengViewModel : BindableBase
    {

        List<OuPengMessage> msgs;
        public List<OuPengMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = OuPengHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
