...
using Demo.Library.YYTalk;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class YYTalkViewModel : BindableBase
    {

        List<YYTalkMessage> msgs;
        public List<YYTalkMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = YYTalkHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
