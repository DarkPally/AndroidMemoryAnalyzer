...
using Demo.Library.BaiduHi;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class BaiduHiViewModel : BindableBase
    {

        List<BaiduHiMessage> msgs;
        public List<BaiduHiMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = BaiduHiHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
