...
using Demo.Library.Dolphin;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class DolphinViewModel : BindableBase
    {

        List<DolphinMessage> msgs;
        public List<DolphinMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = DolphinHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
