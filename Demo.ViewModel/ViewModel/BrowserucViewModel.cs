...
using Demo.Library.BrowserUC;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class BrowserUCViewModel : BindableBase
    {

        List<BrowserUCMessage> msgs;
        public List<BrowserUCMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = BrowserUCHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
