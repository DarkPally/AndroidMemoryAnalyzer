...
using Demo.Library.MicroMsgTrade;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class MicroMsgTradeViewModel : BindableBase
    {

        List<MicroMsgTradeMessage> msgs;
        public List<MicroMsgTradeMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        Msgs = MicroMsgTradeHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
