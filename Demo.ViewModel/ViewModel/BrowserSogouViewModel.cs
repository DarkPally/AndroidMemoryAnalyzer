...
using Demo.Library.BrowserSogou;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class BrowserSogouViewModel : BindableBase
    {

        List<BrowserSogouMessage> msgs;
        public List<BrowserSogouMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = BrowserSogouHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
