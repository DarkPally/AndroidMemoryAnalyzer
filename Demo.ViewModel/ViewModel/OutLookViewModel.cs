...
using Demo.Library.OutLook;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class OutLookViewModel : BindableBase
    {

        List<OutLookMessage> msgs;
        public List<OutLookMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = OutLookHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
