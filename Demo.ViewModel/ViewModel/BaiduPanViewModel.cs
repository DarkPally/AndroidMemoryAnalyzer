...
using Demo.Library.BaiduPan;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class BaiduPanViewModel : BindableBase
    {

        List<BaiduPanMessage> msgs;
        public List<BaiduPanMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = BaiduPanHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
