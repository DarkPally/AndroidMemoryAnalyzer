...
using Demo.Library.MiTalk;
using AndroidMemoryAnalyzer.HeapAnalyzer;

namespace Demo.Library.ViewModel
{

    public class MiTalkViewModel : BindableBase
    {

        List<MiTalkMessage> msgs;
        public List<MiTalkMessage> Msgs
        {
            get { return msgs; }
            set
            {
            ...
            Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        x.DoWork();
                        Msgs = MiTalkHelper.GetMessages(x);
                        State = "解析完成！";
                    }
                    catch
                    {
