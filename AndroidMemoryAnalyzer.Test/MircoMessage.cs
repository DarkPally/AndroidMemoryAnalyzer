...
{
    static public class MircoMessage
    {
        ...
        static void lookForMessage2(HeapFileAnalyzer analyser)
        {
            var t = analyser.PrimitiveArrayInfos.Where(c => c.StringData != null
                && c.StringData.Contains("投标现场")).ToList();
            ...
            using (FileStream fsWrite = new FileStream(@"result_msg_投标现场.txt", FileMode.OpenOrCreate))
            {
        ...
        static void lookForText(HeapFileAnalyzer analyser)
        {
            var t = analyser.ObjectInstanceInfos
                .Where(c => c.ClassName != null
                    && c.ClassName.Contains("com.tencent.mm.ui.conversation.b"))
                    
                    .ToList();
            using (FileStream fsWrite = new FileStream(@"result_ui.conversation.b.txt", FileMode.OpenOrCreate))
            {
                foreach (var it in t)
                {
                    string line = "———————————————— \r\n";
