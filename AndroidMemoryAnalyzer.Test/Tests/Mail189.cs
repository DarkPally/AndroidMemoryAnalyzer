    ...
    static public class Mail189
    {
        ...
        static void lookForMessage2(HeapFileAnalyzer analyser)
        {
            var t = analyser.PrimitiveArrayInfos.Where(c => c.StringData != null
                && c.StringData.Contains("INBOX")).ToList();
            ...
            using (FileStream fsWrite = new FileStream(@"result_msg_INBOX.txt", FileMode.OpenOrCreate))
            {
        ...
        static void lookForText(HeapFileAnalyzer analyser)
        {
            var t = analyser.ObjectInstanceInfos
                .Where(c => c.ClassName != null
                    && c.ClassName.Contains("com.corp21cn.mailapp.activity.nr"))
                    
                    .ToList();
            using (FileStream fsWrite = new FileStream(@"com.corp21cn.mailapp.activity.nr.txt", FileMode.OpenOrCreate))
            {
                foreach (var it in t)
                {
                    string line = "———————————————— \r\n";
