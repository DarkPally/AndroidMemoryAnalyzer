...
{
    static public class MircoMessageTrade
    {
        ...
        static void lookForMessageFloat(HeapFileAnalyzer analyser)
        {
            var t = analyser.ObjectInstanceInfos.Where(
                c=>c.InstanceFields!=null &&
                    c.InstanceFields.Exists(i=>i.Value.Equals(6.66f))
                ).ToList();
            List<ObjectInstanceInfo> tstring = new List<ObjectInstanceInfo>();
           
            /*
            using (FileStream fsWrite = new FileStream(@"result_msg_红包.txt", FileMode.OpenOrCreate))
            {
        ...
        static void lookForMessage2(HeapFileAnalyzer analyser)
        {
            var t = analyser.PrimitiveArrayInfos.Where(c => c.StringData != null
                && c.StringData.Contains("现")).ToList();
            List<ObjectInstanceInfo> tstring = new List<ObjectInstanceInfo>();
            foreach (var it in t)
            {
                var temp = analyser.ObjectInstanceInfos.
            ...
            using (FileStream fsWrite = new FileStream(@"result_msg_吕.txt", FileMode.OpenOrCreate))
            {
                foreach (var it_Main in tObjectInfo)
                {
                    foreach (var it in it_Main)
        ...
        static void lookForText(HeapFileAnalyzer analyser)
        {
            var t = analyser.ObjectInstanceInfos
                .Where(c => c.ClassName != null
                    && c.ClassName.Contains("android.webkit.WebViewCore$WebKitHitTest"))
                    
                    .ToList();
            using (FileStream fsWrite = new FileStream(@"android.webkit.WebViewCore$WebKitHitTest.txt", FileMode.OpenOrCreate))
            {
                foreach (var it in t)
                {
                    string line = "———————————————— \r\n";
