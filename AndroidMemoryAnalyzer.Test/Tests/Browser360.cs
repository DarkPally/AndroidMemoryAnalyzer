...
{
    static public class Browser360
    {
        ...
        static void lookForMessage2(HeapFileAnalyzer analyser)
        {
            var t = analyser.PrimitiveArrayInfos.Where(c => c.StringData != null
                && c.StringData.Contains("百度一下")).ToList();

            var t2 = analyser.StringDatas.Where(c => c.Key == -1478340600).ToList();
            List<ObjectInstanceInfo> tstring = new List<ObjectInstanceInfo>();
            List<ObjectArrayInfo> tstringArray = new List<ObjectArrayInfo>();
            foreach (var it in t)
            {
                var temp = analyser.ObjectInstanceInfos.
                    Where(c => c.InstanceFields != null
            ...
            foreach (var it in t)
            {
                var temp = analyser.ObjectArrayInfos.
                    Where(c => c.ElementIDs != null
                        && c.ElementIDs.Exists(i => i==(it.PrimitiveArrayID)
                        )
                ).FirstOrDefault();
                if (temp != null)
                    tstringArray.Add(temp);
            }
            ...
            using (FileStream fsWrite = new FileStream(@"result_msg_百度一下2.txt", FileMode.OpenOrCreate))
            {
        ...
        static void lookForText(HeapFileAnalyzer analyser)
        {
            ...
            //com.qihoo.browser.model.RecordInfo
            //com.qihoo.browser.navigation.UrlInfo
            //com.qihoo.browser.download.DownloadNotification
            var t = analyser.ObjectInstanceInfos
                .Where(c => c.ClassName != null
                    && c.ClassName.Contains("com.qihoo.browser.model.RecordInfo"))
                    
                    .ToList();
            using (FileStream fsWrite = new FileStream(@"com.qihoo.browser.model.RecordInfo2.txt", FileMode.OpenOrCreate))
            {
                foreach (var it in t)
                {
                    string line = "———————————————— \r\n";
